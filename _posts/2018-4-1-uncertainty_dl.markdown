...
Note that this is in no way a criticism of MC dropout, it's simply pointing out gaps in my individual or our collective knowledge. Nevertheless, its also worth pointing out at an actual criticism of the method in a [note](http://bayesiandeeplearning.org/2016/papers/BDL_4.pdf) by Ian Osband which raises a confusion between risk and model uncertainty and posits that MC dropout is actually approximating risk which if it is the case, shouldn't be the uncertainty we should be putting our money on. I feel that MC dropout is as valid as VI in estimating predictive uncertainty from playing around with the two but the question requires more research hours especially into the theoretical aspects to come to a conclusion.
